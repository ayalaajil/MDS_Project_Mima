...
# --- Step 1: Determine the number of symptoms using a truncated Poisson (or a custom distribution)
def truncated_poisson(lam=1.5, min_val=1, max_val=5):
    """
    Sample from a truncated Poisson distribution where the outcome is at least min_val and at most max_val.
    """
    while True:
...
for i in range(num_phrases):
    ...
    num_symptoms = truncated_poisson(lam=1.5, min_val=1, max_val=len(symptom_list))
...
df.to_csv("dataset_generated_multiple_symptoms_per_phrase_poisson_distrib.csv", index=False)

print("Dataset of 1000 phrases generated and saved to 'dataset_generated_multiple_symptoms_per_phrase_poisson_distrib.csv'.")



# df2 = pd.read_csv(r'dataset_generated_multiple_symptoms_per_phrase_poisson_distrib.csv')

# ## Check All the symptoms are here 
# import ast

# all_symptoms = []
# for entry in df2['Symptoms'].values:
